    ...
    internal class SunMoonPatches
    {
            ...
            ILGenerator generator)
        {
            ...
            foreach (var instruction in instructions)
            {
                ...
                // Look for the target method call
                if (!instruction.Calls(StandardShaderTextureSetter)) continue;
                
                // Duplicate the object and call the desired method
                yield return new CodeInstruction(OpCodes.Dup); // Duplicates object on the stack
                if (RenderCallsiteMethod != null)
                {
                    yield return new CodeInstruction(OpCodes.Call, RenderCallsiteMethod); // Calls the method
                }
                else
                {
                    throw new NullReferenceException("RenderCallsiteMethod is null.");
                }
                found = true;
            }
